...
def main():
    ...
    model_output_type = (
        "pred_xstart"  # model output type: pred_x_prev; pred_xstart; epsilon; score
    )
    ...
    gamma = (
        1 / 100
    )  # coef for iterative sr solver 20steps: 0.05-0.10 for zeta=1, 0.09-0.13 for zeta=0
    ...
    noise_model_t = utils_model.find_nearest(
        reduced_alpha_cumprod, 2 * noise_level_model
    )
    noise_model_t = 0

    noise_inti_img = 50 / 255
    t_start = utils_model.find_nearest(
        reduced_alpha_cumprod, 2 * noise_inti_img
    )  # start timestep of the diffusion process
    ...
    logger.info(
        "model_name:{}, sr_mode:{}, image sigma:{:.3f}, model sigma:{:.3f}".format(
            model_name, sr_mode, noise_level_img, noise_level_model
        )
    )
    logger.info(
        "eta:{:.3f}, zeta:{:.3f}, lambda:{:.3f}, guidance_scale:{:.2f} ".format(
            eta, zeta, lambda_, guidance_scale
        )
    )
    logger.info(
        "start step:{}, skip_type:{}, skip interval:{}, skipstep analytic steps:{}".format(
            t_start, skip_type, skip, noise_model_t
        )
    )
    ...
    for sf in test_sf:
        ...
        for k_index in range(k_num):
            ...
            def test_rho(
                lambda_=lambda_, zeta=zeta, model_output_type=model_output_type
            ):
                logger.info(
                ...
                for idx, img in enumerate(L_paths):
                    ...
                    if sr_mode == "blur":
                        ...
                        img_H_tensor = torch.from_numpy(img_H_tensor)[None, :, :, :].to(
                            device
                        )
                    ...
                    img_L = img_L * 2 - 1
                    img_L += np.random.normal(
                        0, noise_level_img * 2, img_L.shape
                    )  # add AWGN
                    img_L = img_L / 2 + 0.5
                    ...
                    for i in range(num_train_timesteps):
                        sigmas.append(
                            reduced_alpha_cumprod[num_train_timesteps - 1 - i]
                        )
                        if (
                            model_out_type == "pred_xstart"
                            and generate_mode == "DiffPIR"
                        ):
                            sigma_ks.append(
                                (sqrt_1m_alphas_cumprod[i] / sqrt_alphas_cumprod[i])
                            )
                    ...
                    for i in range(len(seq)):
                        ...
                        t_i = utils_model.find_nearest(
                            reduced_alpha_cumprod, curr_sigma
                        )
                        # skip iters
                        if t_i > t_start:
                            continue
                        ...
                        for u in range(iter_num_U):
                            ...
                            if seq[i] != seq[-1]:
                                if generate_mode == "DiffPIR":
                                    if sub_1_analytic:
                                        ...
                                        # first order solver
                                        down_sample = Resizer(x.shape, 1 / sf).to(
                                            device
                                        )
                                    ...
                                    if generate_mode == "DPS_y0":
                                        ...
                                        x = xt - norm_grad * 1.0  # norm / (2*rhos[t_i])
                                        x = x.detach_()
                                        pass
                                    elif generate_mode == "DPS_yt":
                                        y_t = sqrt_alphas_cumprod[t_i] * (
                                            2 * y - 1
                            ...
                            if (
                                ...
                                and model_out_type == "pred_xstart"
                            ) and not (seq[i] == seq[-1] and u == iter_num_U - 1):
                            ...
                            if u < iter_num_U - 1 and seq[i] != seq[-1]:
                                ...
                                sqrt_alpha_effective = (
                                    sqrt_alphas_cumprod[t_i]
                                    / sqrt_alphas_cumprod[t_im1]
                                )
                    ...
                    if calc_LPIPS:
                        ...
                        logger.info(
                            "{:->4d}--> {:>10s} -- sf:{:>1d} --k:{:>2d} PSNR: {:.4f}dB".format(
                                idx + 1, img_name + ext, sf, k_index, psnr
                            )
                        )
                    ...
                    if save_progressive:
                        ...
                        util.imsave(
                            ...
                            os.path.join(
                                E_path,
                                img_name
                                + "_sigma_{:.3f}_process_lambda_{:.3f}_{}_psnr_{:.4f}{}".format(
                ...
                if n_channels == 3:  # RGB image
                    ave_psnr_y_k = sum(test_results["psnr_y"]) / len(
                        test_results["psnr_y"]
                    )
                    logger.info(
                ...
                if calc_LPIPS:
                    ave_lpips_k = sum(test_results["lpips"]) / len(
                        test_results["lpips"]
                    )
                    logger.info(
            ...
            for lambda_ in lambdas:
                ...
                for zeta_i in [0.25]:
                    test_results_ave = test_rho(
                        lambda_, zeta=zeta_i, model_output_type=model_output_type
                    )
    ...
    ave_psnr_sf_k = sum(test_results_ave["psnr_sf_k"]) / len(
        test_results_ave["psnr_sf_k"]
    )
    logger.info(
        "------> Average PSNR of ({}) {:.4f} dB".format(testset_name, ave_psnr_sf_k)
    )
    if n_channels == 3:
        ave_psnr_y_sf_k = sum(test_results_ave["psnr_y_sf_k"]) / len(
            test_results_ave["psnr_y_sf_k"]
        )
        logger.info(
            "------> Average PSNR-Y of ({}) {:.4f} dB".format(
                testset_name, ave_psnr_y_sf_k
            )
        )
    if calc_LPIPS:
        ave_lpips_sf_k = sum(test_results_ave["lpips"]) / len(test_results_ave["lpips"])
        logger.info(
            "------> Average LPIPS of ({}) {:.4f}".format(testset_name, ave_lpips_sf_k)
        )
