...
def _clone_config(base: Config) -> Config:
    # Naive recursive copy via dict round-trip
    def to_dict(obj):
...
def run_methods(opt_path: str, methods: List[str]) -> None:
    ...
    for method in methods:
        ...
        if cfg.task == "sr":
            cfg.result_name += f"_{cfg.sr_mode}{str(cfg.sf)}"

        print(f"=== Running {method} for SR with config: {opt_path} ===")
        run_with_config(cfg)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare SR methods: DiffPIR vs DPS variants."
    )
    parser.add_argument(
        "--opt",
        type=str,
        default="configs/sisr.yaml",
        help="Path to the base YAML config for SR.",
    )
    parser.add_argument(
        "--methods",
        type=str,
