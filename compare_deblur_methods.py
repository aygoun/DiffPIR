...
def run_methods(opt_path: str, methods):
    ...
    for method in methods:
        ...
        cfg.result_name = (
            ...
            f"_blurmode_{cfg.blur_mode}"
        )

        print(f"=== Running {method} for deblurring with config: {opt_path} ===")
        run_with_config(cfg)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare deblurring methods: DiffPIR vs DPS variants."
    )
    parser.add_argument(
        "--opt",
        type=str,
        default="configs/deblur.yaml",
        help="Path to the base YAML config for deblurring.",
    )
    parser.add_argument(
        "--methods",
        type=str,
